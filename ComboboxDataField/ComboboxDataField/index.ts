...
import { 
    F9ComboboxDataField, 
    F9ComboboxDataFieldProps,
    OptionOnSelectData
} from "./F9ComboboxDataField";
import {
    F9SimpleOption,
    F9DataSetOption,
    parseOptions,
...
export class ComboboxDataField implements ComponentFramework.ReactControl<IInputs, IOutputs> {
    ...
    private items: IInputs["Items"];
    private itemOptions: F9DataSetOption<OptionProps>[];
    ...
    private onChange(targetRef: React.RefObject<HTMLInputElement>, data: OptionOnSelectData) {
            ...
            if(!this.items.loading){
                this.items.setSelectedRecordIds(
    ...
    public init(
        ...
        this.items = context.parameters.Items;
    ...
    public updateView(context: CustomContext<IInputs>): React.ReactElement {
        ...
        this.items = Items;
        if(!Items.loading){
            const itemOptions = Items.sortedRecordIds.map((recordId) => {
                const record = Items.records[recordId];
                const option = {
                    ...record,
                    Key: recordId,
                    Text: (record.getValue("Text") || record.getValue("Value") || recordId).toString()
                }
                return option;
            })
        }
        ...
        const props: F9ComboboxDataFieldProps = { 
            /* field props */
            fieldProps: {
                label: this.label,
